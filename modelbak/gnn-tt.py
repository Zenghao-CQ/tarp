...
def draw_fig(list,name,epoch):
    ...
    if name=="loss":
        ...
        # plt.grid()
        plt.savefig("./Vlid_accuracy.png")
        f2=open("./val_acc.csv",mode='a')
        f2.write(",".join([str(x) for x in y1]))
        f2.close()
...
# gl2,l = load_graphs('../dgl_graph/40.g')
# glist.extend(gl2)
...
if __name__ == "__main__":
    th.manual_seed(3407)
    # print(len(glist))
    train(glist,val_list, model, num_epoch=350, learning_rate=0.00002)
    # g=glist[100]
