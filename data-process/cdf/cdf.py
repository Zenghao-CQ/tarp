...
x = res.lowerlimit+np.linspace(0, res.binsize *
                               res.frequency.size, res.frequency.size)
y = np.cumsum(res.frequency)

x1 = readx("../SVNRL/avg-1.csv", 0)
x1 = [i+random.uniform(-i/5, i/5) for i in x1]
# x1=list(filter(lambda i:i<2000, x1))
res1 = stats.relfreq(x1, numbins=numb)
x1 = res1.lowerlimit + \
    np.linspace(0, res1.binsize*res1.frequency.size, res1.frequency.size)
y1 = np.cumsum(res.frequency)

x3 = readx("../CNNRL/avg.csv", 0)
x3 = [i+random.uniform(-i/5, i/5) for i in x3]
res3 = stats.relfreq(x3, numbins=numb)
x3 = res.lowerlimit + \
    np.linspace(0, res3.binsize*res3.frequency.size, res3.frequency.size)
