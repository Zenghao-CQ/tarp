...
class DDPG:
    def __init__(self, env):
        ...
        self.resDim = NUM_RES
        self.actor = Actor(self.stateDim, self.resDim, self.actionDim)
        self.critic = Critic(self.stateDim, self.actionDim)
        self.targetActor = deepcopy(Actor(self.stateDim, self.resDim, self.actionDim))
    ...
    def train(self):
        ...
        for episode in range(self.start, self.end):
            ...
            for step in range(NUM_TIMESTEPS):
                g,d,tp,SLO_score,res_use = self.env.get_state() #state_graph, reawrd, delay, SLO, resource usage
                ...
                self.actor.eval()     
                action, actionToBuffer = self.getMaxAction(g)
                ...
                # step episode
                nextg,reward,d,tp,SLO_score,res_use=self.env.perform_action(prof) #state_graph, reawrd, delay, SLO, resource usage
                ...
                self.tpgraph.append(tp)
                self.delay2.append(d[1])
                self.delay1.append(d[0])
                # Update replay bufer
                self.replayBuffer.append((g, actionToBuffer, nextg, reward))
                ...
                if len(self.replayBuffer) >= self.warmup:
                    ...
                    curStateBatch = dgl.batch(curStateBatch) # merge into big graph
                    nextStateBatch = dgl.batch(nextStateBatch)
            ...
            if PLOT_FIG:
                ...
                if  episode % delta ==0 and episode != 0:
                    ...
                    plt.cla()
                    plt.plot(self.tpgraph, color='darkorange')  # total resource usage in an iteration or episode
                    # plt.plot(avg_rewards, color='b')  # (moving avg) rewards
                    plt.xlabel('Steps')
                    plt.savefig('./fig/ep'+str(episode)+'-tp.png')
                    ...
                    f2.close()                   
                    f2=open("./logs/tp-"+str(episode)+".csv",mode='w')
                    f2.write(",".join([str(x) for x in self.tpgraph]))
                    f2.close()
        if PLOT_FIG:
            plt.cla()
            plt.plot(self.rewardgraph, color='darkorange')  # total rewards in an iteration or episode
