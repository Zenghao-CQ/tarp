...
class Actor(nn.Module):    
    def __init__(self, stateDim,resDim, actionDim): #state Dim = Node feature size
        ...
        self.bn1 = nn.BatchNorm1d(GCONV1)
                                    
        self.fc2 = nn.Linear(GCONV1, HID_LAYER2)
        ...
        self.bn2 = nn.BatchNorm1d(GCONV1+HID_LAYER3)

        self.fc3 = nn.Linear(GCONV1+HID_LAYER3, self.actionDim)
    ...
    def forward(self, g): #ip delay+N, ipres-> resource ie. N
        ip = g.ndata["N_DELAY"]
        F.normalize(ip.float(),p=1,dim=-1)
        ipres = g.ndata["N"]
        ...
        h2 = torch.cat([h1_norm, hres],dim=1)
        h2_norm = self.bn2(h2)
        #print(h2.shape)                        
